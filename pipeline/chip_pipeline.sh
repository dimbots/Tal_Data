...
mkdir fastqc | mv *.zip *.html fastqc/
mkdir trimmed | mv *.T.fastq.gz *.log trimmed/
...
ln -s normalized/*.bw .
	...
	for bw in $(ls *.bw)
		...
		plotHeatmap -m $out_matrix -out $out_plot --colorMap YlGnBu --boxAroundHeatmaps no --missingDataColor 1
	done

fi
mkdir heatmaps | mv *.png *.out_matrix *.out_regions heatmaps
rm *.bw
